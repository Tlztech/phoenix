...
from service.common.uniqlo_common import sprider
from util import log_util, excel_util, env_util, mail_util, common_utils


def service():
    log_util.info(f"品牌{brand.UNIQLO_BRAND}补货检查脚本processing")
    ...
    if not item_codes:
        ...
        log_util.info(f"品牌{brand.UNIQLO_BRAND}补货检查脚本processed")
    elif not targets:
        ...
        try:
                    ...
                    if output_data_list and len(output_data_list) > 0:
                        ...
                        for index, output_data in enumerate(output_data_list):
                            if prices[index] and int(prices[index]) > 0:
                                ...
                                elif int(prices[index]) >= common_utils.prices_formatter(output_data['price']) and output_data['官网库存'] and output_data['官网库存'] == '在庫あり':
            ...
            log_util.info(f"品牌{brand.UNIQLO_BRAND}价格变动检查脚本processed")