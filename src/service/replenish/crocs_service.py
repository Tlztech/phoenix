...
from service.common.crocs_common import sprider
from util import log_util, excel_util, env_util, mail_util, common_utils


def service():
    log_util.info(f"品牌{brand.CROCS_BRAND}补货检查脚本processing")
    ...
    if not item_codes:
        ...
        log_util.info(f"品牌{brand.CROCS_BRAND}补货检查脚本processed")
    elif not targets:
        ...
        try:
                    ...
                    if output_data_list and len(output_data_list) > 0:
                        ...
                        for output_data in output_data_list:
                            for index, url in enumerate(item_codes):
                                if output_data['url'] != url:
                                    continue
                                if sizes[index] and output_data['size'] and sizes[index] == output_data['size'] and colors[index] and output_data['color'] and colors[index] == output_data['color'] and prices[index] and output_data['price']:
                                    if output_data['官网库存'] and output_data['官网库存'] != '在庫なし' and int(prices[index]) >= common_utils.prices_formatter(output_data['price']):
                                        output_mail_list.append(output_data)
                                    elif output_data['官网库存']:
                                        item_codes_list.append(output_data['url'])
                                        price_list.append(prices[index])
                                        color_list.append(colors[index])
                                        size_list.append(sizes[index])
                                    else:
                                        error_stock_list.append(output_data['url'])
                                    break
                                elif not output_data['color']:
                                    error_color_list.append(output_data['url'])
                                elif not output_data['size']:
                                    error_size_list.append(output_data['url'])
                                elif not output_data['price']:
                        ...
                        item_codes = copy.deepcopy(item_codes_list)
                        # mail
                        ...
                        if error_color_list:
                            error_msg = f"color获取失败，请确认这些商品网页{':'.join(error_color_list)}"
                            log_util.error(error_msg)
                            # notify error with mail
                            mail_util.send_message("补货检查处理color获取失败错误信息", error_msg)
                        if error_size_list:
                            error_msg = f"size获取失败，请确认这些商品网页{':'.join(error_size_list)}"
                            log_util.error(error_msg)
                            # notify error with mail
                            mail_util.send_message("补货检查处理size获取失败错误信息", error_msg)
                        if error_price_list:
                            error_msg = f"价格获取失败，请确认这些商品网页{':'.join(error_price_list)}"
                            log_util.error(error_msg)
                            # notify error with mail
                            mail_util.send_message("补货检查处理价格获取失败错误信息", error_msg)
                        if error_stock_list:
                            error_msg = f"库存获取失败，请确认这些商品网页{':'.join(error_stock_list)}"
                            log_util.error(error_msg)
                            # notify error with mail
                            mail_util.send_message("补货检查处理库存获取失败错误信息", error_msg)
                    else:
            ...
            log_util.info(f"品牌{brand.CROCS_BRAND}价格变动检查脚本processed")
