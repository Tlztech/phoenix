...
from service.common.descente_common import sprider
from util import log_util, excel_util, env_util, mail_util, common_utils


def service():
    log_util.info(f"品牌{brand.DESCENTE_BRAND}补货检查脚本processing")
    ...
    if not item_codes:
        ...
        log_util.info(f"品牌{brand.DESCENTE_BRAND}补货检查脚本processed")
    elif not targets:
        ...
        try:
                        ...
                        for output_data in output_data_list:
                            for index, url in enumerate(item_codes):
                                if output_data['url'] == url and sizes[index] and output_data['size'] and sizes[index] == output_data['size']:
                                    if output_data['官网库存'] and output_data['官网库存'] != 'なし':
            ...
            log_util.info(f"品牌{brand.DESCENTE_BRAND}价格变动检查脚本processed")
