...
from service.common.keen_common import sprider
from util import log_util, excel_util, env_util, mail_util, common_utils


def service():
    log_util.info(f"品牌{brand.KEEN_BRAND}补货检查脚本processing")
    ...
    if not item_codes:
        ...
        log_util.info(f"品牌{brand.KEEN_BRAND}补货检查脚本processed")
    elif not targets:
        ...
        try:
            while interval >= 0 and item_codes:
                try:
                    ...
                    if output_data_list and len(output_data_list) > 0:
                        ...
                        if output_mail_list:
                            byte_data = common_utils.generate_excel_friendly_csv(
                                common_utils.transform_dict_to_list(output_mail_list))
            ...
            log_util.info(f"品牌{brand.KEEN_BRAND}价格变动检查脚本processed")
