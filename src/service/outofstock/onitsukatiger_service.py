...
from service.common.onitsukatiger_common import sprider
from util import log_util, excel_util, env_util, mail_util, common_utils


def service():
    log_util.info(f"品牌{brand.ONITSUKATIGER_BRAND}断货检查脚本processing")
    ...
    if not item_codes:
        ...
        mail_util.send_message("断货检查处理错误信息", error_msg)
        log_util.info(f"品牌{brand.ONITSUKATIGER_BRAND}断货检查脚本processed")
    elif not targets:
        ...
        try:
                ...
                try:
                    ...
                    if output_data_list and len(output_data_list) > 0:
                        ...
                        for output_data in output_data_list:
                            for index, url in enumerate(item_codes):
                                if output_data['url'] == url and sizes[index] and output_data['size'] and sizes[index] == output_data['size']:
                                    if output_data['官网库存'] and output_data['官网库存'] == '无货':
                        ...
                        if output_mail_list:
                            ...
                            mail_util.send_csv_attachment("断货检查", byte_data)
                        if error_price_list:
                            error_msg = f"价格获取失败，请确认这些商品网页{':'.join(error_price_list)}"
                            log_util.error(error_msg)
                            # notify error with mail
                            mail_util.send_message("断货检查处理价格获取失败错误信息", error_msg)
                        if error_stock_list:
                            error_msg = f"库存获取失败，请确认这些商品网页{':'.join(error_stock_list)}"
                            log_util.error(error_msg)
                            # notify error with mail
                            mail_util.send_message("断货检查处理库存获取失败错误信息", error_msg)
                    else:
            ...
            log_util.info(f"品牌{brand.ONITSUKATIGER_BRAND}价格变动检查脚本processed")
