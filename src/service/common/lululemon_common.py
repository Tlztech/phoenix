            ...
            for target in targets:
                if target in lululemon_hosts:
                    ...
                    for item_code in item_codes:
                        ...
                        item_data_list = []
                        item_data = {'url': item_code}
                        for action_l_1 in actions:
                            if action_l_1['action_type'] == 'dynamic':
                                try:
                                    ...
                                    if action_l_1['url'] and action_l_1['action'] == 'get':
                                        ...
                                        driver.get(action_l_1['url'])
                                        item_data.update({'color': '', 'size': '', 'price': ''})
                                        try:
                                            current_path = list(action_l_1['path'].values())[0]
                                            driver.find_element(By.XPATH, current_path)
                                            item_data.update({'color': "商品不存在"})
                                            ...
                                            # color
                                            try:
                                                current_path = list(action_l_1['path'].values())[1]
                                                color_element = WebDriverWait(driver, 10).until(
                                                    EC.presence_of_element_located((By.XPATH, current_path))
                                                )
                                                color = color_element.get_attribute(list(action_l_1['path'].keys())[1])
