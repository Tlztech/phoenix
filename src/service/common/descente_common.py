...
def sprider(item_codes, targets):
    ...
    try:
        if item_codes and targets:
            converse_hosts = yaml_util.get_brand_hosts(brand.DESCENTE_BRAND)
            for target in targets:
                if target in converse_hosts:
                    log_util.info(f"网站{target}脚本processing")
                    for item_code in item_codes:
                        actions = yaml_util.get_object_price_actions_top(brand=brand.DESCENTE_BRAND, host=target)
                        item_data_list = []
                            ...
                            if action['action_type'] == 'dynamic':
                                ...
                                if action['action'] == "get":
                                    ...
                                    try:
                                        ...
                                        log_util.error(f"商品{item_code} size，库存获取失败:{''.join(traceback.format_exception(None, te, te.__traceback__))}")
                                        size = ''
                                        stock = ''
                                    if elements:
                                        for element in elements:
                                            ...
                                            try:
                                                ...
                                                stock = ''
                                            item_data = {'url': item_code, 'size': size, '官网库存': stock, 'price': price}
                    ...
                    log_util.info(f"{brand.DESCENTE_BRAND}品牌没有{env_util.get_env('EXCEL_INPUT_FILE')}文件指定{target}网站脚本定义")
