...
def sprider(item_codes, targets):
    ...
    try:
        if item_codes and targets:
            kagami_hosts = yaml_util.get_brand_hosts(brand.KAGAMI_BRAND)
            for target in targets:
                if target in kagami_hosts:
                    log_util.info(f"网站{target}脚本processing")
                    for item_code in item_codes:
                        actions = yaml_util.get_object_price_actions_top(brand=brand.KAGAMI_BRAND, host=target)
                        item_data_list = []
                    ...
                    log_util.info(f"{brand.KAGAMI_BRAND}品牌没有{env_util.get_env('EXCEL_INPUT_FILE')}文件指定{target}网站脚本定义")
