        ...
        if dewu_formatted_model in tianmao_model_list:
            ...
            for dewu in dewu_value:
                ...
                if re.search(r'\bsize\b', temp_value, re.IGNORECASE):
                    ...
                    if match:
                        ...
                        if remaining_cleaned and color_is_empty:
                            color_value = remaining_cleaned
                            print(f"条件满足，写入color: '{remaining_cleaned}' ")
                        else:
                            if not remaining_cleaned:
                                print(f" 剩余文本为空，不写入color")
                            elif not color_is_empty:
                ...
                for tianmao in tianmao_value:
                    print(f"color_value: '{color_value}', size_word: '{size_word}'")
                    if (((  color_value is not None and size_word is not None) and (
                            tianmao.get(excel.TIANMAO_COLUMN_INDEX.get('color'))== color_value ) and (
                            tianmao.get(excel.TIANMAO_COLUMN_INDEX.get('size')) == size_word)) or (
                            
                            ((  color_value is None and size_word is not None) and (
                            tianmao.get(excel.TIANMAO_COLUMN_INDEX.get('size')) == size_word))) or (
                            
                            ((  color_value is not None and size_word is None) and (
                            tianmao.get(excel.TIANMAO_COLUMN_INDEX.get('color')) == color_value)))
                        ):
